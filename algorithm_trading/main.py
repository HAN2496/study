...
class User:
    ...
    def trade(self, ticker, predict, trade_time):
        ...
        """
        #장 열리는 시간이 넘어서까지 계산이 안끝난 경우를 방지하기 위함
        if trade_time.hour > 9:
            return trade_time

        #장 오픈 전에 trade코드로 넘어왔으면 해당 날짜의 open value를 가져오면 됨
        transaction_time = trade_time.date()
        predicted = predict[1] - predict[0] #0이 open, 1이 close

        # 예측 가격이 현재 가격 대비 목표 수익률 이상 증가할 것으로 예상되면 구매
        current_price = pyupbit.get_ohlcv(ticker, interval='minute1', count=1, to=transaction_time).iloc[0]['open']
        if predicted >= 0:
            # 예상 수익률 달성을 위한 구매 수량 계산
            quantity_to_buy = self.asset.cash // current_price
            if quantity_to_buy > 0:
                self.buy_crypto(ticker, quantity_to_buy, current_price, transaction_time)
        ...
        # 예측 가격이 현재 가격보다 낮을 것으로 예상되면 판매
            ...
            if ticker in self.asset.portfolio and self.asset.portfolio[ticker][0] > 0:
                ...
                self.sell_crypto(ticker, quantity_to_sell, current_price, transaction_time)
                print(f"Sold {quantity_to_sell} units of {ticker} at {current_price} each.")
        return trade_time + timedelta(minutes=10)
"""
Virtual Trading
예측 값은 close 가격
...
#데이터셋 생성 함수
def create_dataset(data, time_step, start_idx):
    X, Y = [], []
    for i in range(len(data)-time_step-1):
        X.append(data[i + start_idx:i + time_step + start_idx, 0:5])
        Y.append([data[i + time_step, 0], data[i + time_step, 3]])  # Open과 Close 가격
    return np.array(X), np.array(Y)

def for_virtual_time(virtual_time):
    if virtual_time.hour < 9:
        new_time = virtual_time.replace(hour=8, minute=30, second=0, microsecond=0) + timedelta(days=1)
    else:
        new_time = virtual_time.replace(hour=8, minute=30, second=0, microsecond=0) + timedelta(days=1)
    return new_time
...
def main():
    ...
    #가상 시간 설정
    virtual_time_now = for_virtual_time(day_ago)

    df = fetch_data_virtual_trading(ticker=ticker, interval=interval, start_date=day_ago)
    print(f"Total Data - Expect: {test_days * 24 * 4} / Real: {len(df)}")
    scaled_data, scaler = preprocess_data(df)

    # 500개 데이터로 예측
    time_step = 1000

    for i in range(int(test_days * 24 * 60 / 15 / time_step)):
        virtual_time_now = for_virtual_time(virtual_time_now)
        print(f"Virtual time now: {virtual_time_now}")
        # 데이터셋 준비
        X, Y = create_dataset(scaled_data, time_step, i)
        print(np.shape(X), np.shape(Y)) #X는 ohlcv 5개, Y는 oc 2개
        ...
        save_history(history, i, name, show=True)

        predictions = model.model.predict(X_test)
        print(predictions.shape)
        predictions = scaler.inverse_transform(predictions)  # 예측값을 원래 스케일로 변환

        # 여기서 current_price를 설정
        current_price = scaler.inverse_transform(X_test[-1, -1, :].reshape(1, -1))[0, 3]  # 마지막 데이터 포인트의 종가

        virtual_time_now = user.trade(ticker=ticker, predict=predictions, trade_time=virtual_time_now)
